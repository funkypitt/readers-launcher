...
* Built-in widgets, all of a fixed height so nothing on the page ever shifts: **clock**,
  ...
  **agenda** (next event, swipe for the following ones), **tasks** (a Tasks.org list:
  ...
  record with one tap), **recordings** (listen, swipe through them), **food log** (a shortcut
  to Reader's Food Log's viewfinder). Any standard app widget can be
...
| the food log tile | a shortcut to [Reader's Food Log](https://github.com/funkypitt/readers-foodlog)'s viewfinder: one touch for a new photo, nothing else |
