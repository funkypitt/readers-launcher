...
* Built-in widgets, all of a fixed height so nothing on the page ever shifts: **clock**,
  ...
  **agenda** (next event, swipe for the following ones), **tasks** (a Tasks.org list:
  ...
  record with one tap), **recordings** (listen, swipe through them), **food log** (Reader's Food
  Log: today's photos, ✎ activity, ⚖ weight, ◉ camera). Any standard app widget can be
...
| the recordings tile | listen to [Reader's Recorder](https://github.com/funkypitt/readers-recorder)'s recordings: the latest one, swipe left for the older ones; ▶ plays through the app's player (❚❚ pauses, position / length shown), the text opens the recording |
| the food log tile | [Reader's Food Log](https://github.com/funkypitt/readers-foodlog) 1.2.0+: today's photos on the first line, activities and weight under it; ✎ notes a physical activity, ⚖ asks for the day's weight (and shows the curve), ◉ opens the viewfinder; the text opens the journal |
